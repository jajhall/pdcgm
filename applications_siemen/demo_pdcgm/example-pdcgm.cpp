...
int main(void)
{
    ...
    short  *row_type; /* type of each constraint in the master problem */

    std::vector<double> b;
    std::vector<double> lo_box;
    std::vector<double> up_box;

    y0 = PDCGM_ALLOC(double,dim+nb_cvxty_constr+1);
    row_type = PDCGM_ALLOC(short, dim+nb_cvxty_constr+1);
    if(row_type==NULL) return -1;
    ...
    /* Set the objective function costs */
    b.push_back(0.0);
    row_type[dim + nb_cvxty_constr] = OBJECTIVE;
    ...
    PDCGM_env.PDCGM_set_verbosity(3);
