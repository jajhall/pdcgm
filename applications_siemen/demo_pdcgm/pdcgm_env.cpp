...
int PDCGM_env::PDCGM_set_data(
    	...
    	void   *instance_data)
   {
	...
	if(lo_box.empty())
	{
		G.get()->PDCGM_set_dual_lb(dim,-PDCGM_BIG);
		G.get()->PDCGM_set_dual_lb_fxd(dim,-PDCGM_BIG);
	}
	else
	{
	...
	if(up_box.empty())
	{
		G.get()->PDCGM_set_dual_ub(dim,PDCGM_BIG);
		G.get()->PDCGM_set_dual_ub_fxd(dim,PDCGM_BIG);
	}
	else
	{
